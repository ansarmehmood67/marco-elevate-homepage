...
import { ArrowRight, Users, PhoneCall, Headphones, Megaphone, TrendingUp, Youtube, UserRound, Bot, Workflow, Globe, Cloud, Plug } from "lucide-react";
...
const ValuePropositionSection = () => {
  ...
  const services = [
    ...
    { 
      ...
      icon: Megaphone,
      accent: "violet", // Consulenza category
  ...
  const getAccentClasses = (accent: string) => {
    switch (accent) {
      ...
      case 'violet':
        return {
          badge: 'bg-violet-500/10 group-hover:bg-violet-500/20',
          ring: 'ring-violet-500/20 group-hover:ring-violet-500/40',
          button: 'from-violet-500 to-violet-600'
        };
      case 'green':
        return {
          badge: 'bg-emerald-500/10 group-hover:bg-emerald-500/20',
          ring: 'ring-emerald-500/20 group-hover:ring-emerald-500/40',
          button: 'from-emerald-500 to-emerald-600'
        };
      default:
        return {
          badge: 'bg-white/10 group-hover:bg-white/20',
          ring: 'ring-white/20 group-hover:ring-white/40',
          button: 'from-primary to-primary-glow'
        };
    }
  };
        ...
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 lg:gap-10">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className={`group relative bg-white/6 backdrop-blur-xl rounded-xl p-8 ring-1 ring-white/10 shadow-sm transition-all duration-500 cursor-pointer hover:shadow-lg hover:-translate-y-2 hover:scale-105 ${
              ...
              >
                {/* Clean Icon Container with Subtle Tinted Badge */}
                <div className={`relative mb-8 w-14 h-14 rounded-xl ${accentClasses.badge} ring-1 ${accentClasses.ring} flex items-center justify-center shadow-sm transition-all duration-300`}>
                  <IconComponent className="w-8 h-8 text-white/90 stroke-[1.75] transition-all duration-300" />
                </div>
                ...
                {/* Clean Hover Button */}
                <div className={`relative mt-8 transition-all duration-500 ${
                  hoveredIndex === index ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}>
                  <button className={`w-full flex items-center justify-center space-x-2 bg-gradient-to-r ${accentClasses.button} text-white px-6 py-3 rounded-lg font-semibold text-sm shadow-lg hover:shadow-xl transition-all duration-300 group/btn`}>
